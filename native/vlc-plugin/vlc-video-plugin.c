...
static bool load_libvlc_module(void)
{
	...
	/* VaultStudio: prefer the libVLC bundled with the app so the Playlist
	 * (VLC) source works on machines without a separate VLC install. The
	 * engine passes VS_RUNTIME_DIR; libVLC + its plugins live in
	 * <runtime>\vlc\. Falls through to a system VLC install (registry)
	 * when the bundle is missing. */
	{
		const char *runtime = getenv("VS_RUNTIME_DIR");
		if (runtime && *runtime) {
			struct dstr libpath = {0};
			struct dstr pluginpath = {0};
			dstr_copy(&libpath, runtime);
			dstr_replace(&libpath, "/", "\\");
			dstr_copy(&pluginpath, libpath.array);
			dstr_cat(&libpath, "\\vlc\\libvlc.dll");
			dstr_cat(&pluginpath, "\\vlc\\plugins");
			if (os_file_exists(libpath.array)) {
				_putenv_s("VLC_PLUGIN_PATH", pluginpath.array);
				libvlc_module = os_dlopen(libpath.array);
			}
			dstr_free(&libpath);
			dstr_free(&pluginpath);
		}
		if (libvlc_module)
			return true;
	}
